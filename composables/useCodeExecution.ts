...
export const useCodeExecution = () => {
  ...
  const executeCode = async (code: string, timeout: number = 10): Promise<ExecutionResult> => {
    ...
    try {
      ...
      const errorMessage = err.data?.message || err.message || 'Error desconocido al ejecutar el código'
  ...
  // Guardar código
  const saveCode = async (code: string, title: string = 'Untitled'): Promise<{ id: string; share_url: string }> => {
    try {
